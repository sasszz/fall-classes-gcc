...
//------------------------------------------------------------
// In class program function
//------------------------------------------------------------
template<class ItemType>
int BinarySearchTreeNode<ItemType>::sumTreeHelper(int subTreePtr) {
    ...
    int leftSum = sumTreeHelper(tree[subTreePtr].getLeft());
    int rightSum = sumTreeHelper(tree[subTreePtr].getRight());

    // Return the sum of the current node's value and the sums of its subtrees
    return tree[subTreePtr].getItem() + leftSum + rightSum;
}

template<class ItemType>
int BinarySearchTreeNode<ItemType>::sumTree() {
    if (root == NIL_NODE) {
        return 0;
    }
