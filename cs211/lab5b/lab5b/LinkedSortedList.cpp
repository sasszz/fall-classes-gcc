...
auto LinkedSortedList<ItemType>::getNodeBefore(const ItemType& anEntry) const
{
    ...
    while ((currentNodePtr != nullptr) && (anEntry.getProductName() > currentNodePtr->getItem().getProductName()))
    {
...
template<class ItemType>
double LinkedSortedList<ItemType>::calculateTotalValue() const
{
    ...
    {
        totalValue += currentPtr->getItem().value();
...
template<class ItemType>
void LinkedSortedList<ItemType>::displayInventory() const
{
    std::shared_ptr<Node<ItemType>> currentPtr = headPtr;

    // Iterate through the linked list
    while (currentPtr != nullptr)
    {
        // Use the overloaded << operator to print the Inventory object
       std::cout << currentPtr->getItem() << std::endl;
