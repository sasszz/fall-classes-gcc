...
auto BinarySearchTreeNode<ItemType>::removeNode(int nodePtr){
    ...
    if (tree[nodePtr].isLeaf())
    {
       ...
       return NIL_NODE; // delete and return nullptr
    }
...
template<class ItemType>
int BinarySearchTreeNode<ItemType>::readTreeHelper(ifstream& infile, int n) {
    int treePtr = NIL_NODE;
    int leftPtr, rightPtr;
    ItemType item;
    if (n > 0) {
        leftPtr = readTreeHelper(infile, n/2);
        getline(infile>>ws, item);
//        infile >> item;
        treePtr = allocateNode(item);
        tree[treePtr].setLeft(leftPtr);
        rightPtr = readTreeHelper(infile, (n-1)/2);
        tree[treePtr].setRight(rightPtr);
    }
    return treePtr;
};

template<class ItemType>
void BinarySearchTreeNode<ItemType>::writeTreeHelper(ofstream& outfile, int subTreePtr) {
    if (subTreePtr != NIL_NODE) {
        writeTreeHelper(outfile, tree[subTreePtr].getLeft());
        outfile << tree[subTreePtr].getItem() << endl;
        writeTreeHelper(outfile, tree[subTreePtr].getRight());
    }
...
template<class ItemType>
void BinarySearchTreeNode<ItemType>::readTree(string filename) {
    ...
    root = readTreeHelper(infile, count);
}

template<class ItemType>
void BinarySearchTreeNode<ItemType>::writeTree(string filename) {
    ofstream outfile(filename);
    writeTreeHelper(outfile, root);
    outfile.close();
...
// EXTRA CREDIT
// Implementation of the readTree function
template<class ItemType>
void BinarySearchTreeNode<ItemType>::readTreeEC(string filename) {
    ifstream infile(filename);
    ItemType item;
    vector<ItemType> sortedItems;
    ...
    root = readTreeHelperEC(sortedItems, 0, count-1);
}

// Implementation of the readTreeHelper function
template<class ItemType>
int BinarySearchTreeNode<ItemType>::readTreeHelperEC(const vector<ItemType>& sortedItems, int start, int end) {
    if (start > end) {
        return NIL_NODE;
    }

    int mid = (start + end) / 2;
    int treePtrIndex = allocateNode(sortedItems[mid]);  // Provide the item when allocating a new node

    // Assuming TreeNode has setLeft and setRight methods
    tree[treePtrIndex].setLeft(readTreeHelperEC(sortedItems, start, mid - 1));
    tree[treePtrIndex].setRight(readTreeHelperEC(sortedItems, mid + 1, end));

    return treePtrIndex;  // Returning the index of the newly created node
}
#endif
